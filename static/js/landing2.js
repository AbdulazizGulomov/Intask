...
function heroView(mode){
  const mapSec=document.getElementById('mapSec'),list=document.getElementById('heroList');
  if(mode==='map'){if(mapSec)mapSec.style.display='block';if(list)list.classList.remove('on');if(hM)setTimeout(()=>hM.invalidateSize(),100)}
  else{if(mapSec)mapSec.style.display='none';if(list)list.classList.add('on');buildMasterGrid()}
  document.querySelectorAll('.vt-btn').forEach(b=>{
    const isMap=b.textContent.includes('Xarita')||b.textContent.includes('Карта');
    const isList=b.textContent.includes("Ro'yxat")||b.textContent.includes('Список');
    b.classList.remove('on');
    if(mode==='map'&&isMap)b.classList.add('on');
    if(mode==='list'&&isList)b.classList.add('on');
  });
}
...
window.addEventListener('DOMContentLoaded',()=>{
  tryI(20);
  startTyping();
  setTimeout(startActivityFeed,4000);
  setTimeout(showFoundPopup,5000);
  initTestCarousel();
  initBASliders();
});
...
function startTyping(){
  const el=document.getElementById('heroTyped');if(!el)return;
  const words=['Usta toping','Elektrik','Santexnik','Ta\'mirchi','Tozalovchi'];
...
// ====== LIVE ACTIVITY FEED ======
const FEED_ITEMS=[
  {av:'А',bg:'#1a56db',text:'<b>Aziz K.</b> hozirgina Yunusobodda buyurtma qabul qildi',time:'2 min oldin'},
  {av:'Д',bg:'#059669',text:'<b>Dmitriy L.</b> Chilanzorda ishni tugatdi',time:'5 min oldin'},
  {av:'М',bg:'#d97706',text:'<b>Malika S.</b> elektrik chaqirdi — 3 min ichida topildi!',time:'8 min oldin'},
  {av:'Б',bg:'#6366f1',text:'<b>Baxrom U.</b> Sergeli\'da plitka yotqizishni boshladi',time:'12 min oldin'},
  {av:'Н',bg:'#dc2626',text:'<b>Nodira A.</b> ★4.9 baho qo\'ydi — "Zo\'r usta!"',time:'15 min oldin'},
  {av:'О',bg:'#8b5cf6',text:'<b>Oleg P.</b> IKEA oshxona mebeli yig\'di',time:'20 min oldin'},
  {av:'Ж',bg:'#059669',text:'<b>Jasur T.</b> Mirzo Ulug\'bekda kran almashtirdi',time:'25 min oldin'},
];
let feedIdx=0;
function startActivityFeed(){
  const cont=document.getElementById('liveFeed');if(!cont)return;
  function showNext(){
    const item=FEED_ITEMS[feedIdx%FEED_ITEMS.length];
    const el=document.createElement('div');el.className='feed-item';
    el.innerHTML=`<div class="feed-av" style="background:${item.bg}">${item.av}</div><div><div class="feed-text">${item.text}</div><div class="feed-time">${item.time}</div></div>`;
    cont.appendChild(el);
    setTimeout(()=>{el.classList.add('out');setTimeout(()=>el.remove(),400)},4000);
    feedIdx++;
    if(cont.children.length>2){const first=cont.children[0];first.classList.add('out');setTimeout(()=>first.remove(),400)}
  }
  showNext();
...
function showFoundPopup(){
  const p=document.getElementById('fpPopup'),o=document.getElementById('fpOverlay');
  if(!p||!o)return;
  p.classList.add('show');o.classList.add('show');
}
function closeFP(){
  const p=document.getElementById('fpPopup'),o=document.getElementById('fpOverlay');
  if(p)p.classList.remove('show');if(o)o.classList.remove('show');
}

// ====== TESTIMONIALS CAROUSEL ======
let testPos=0,testMax=0;
function initTestCarousel(){
  const track=document.getElementById('testTrack');if(!track)return;
  const cards=track.children.length;
  let visible=3;
  if(window.innerWidth<768)visible=1;else if(window.innerWidth<1024)visible=2;
  testMax=Math.max(0,cards-visible);
  updateTestDots();
}
function testMove(dir){
  testPos=Math.max(0,Math.min(testMax,testPos+dir));
  const track=document.getElementById('testTrack');if(!track)return;
  const card=track.children[0];if(!card)return;
  const w=card.offsetWidth+20;
  track.style.transform=`translateX(-${testPos*w}px)`;
  updateTestDots();
}
function testGoTo(i){testPos=i;testMove(0)}
function updateTestDots(){
  document.querySelectorAll('.test-dot').forEach((d,i)=>{d.classList.toggle('on',i===testPos)});
}
...
function closeAnn(){const b=document.getElementById('annBar');if(b){b.style.display='none';document.querySelector('.hdr').style.top='0';document.querySelector('.hero').style.marginTop='64px';const hl=document.querySelector('.hero-list');if(hl)hl.style.marginTop='64px';}}